...
from ._interface import AbstractBaseHaredisManager
from .._client import HaredisClientCmdExecutor 



class BaseHaredisManager(AbstractBaseHaredisManager):
    """## Base Redis Lock Release Manager Class for Distributed Caching/Locking in Redis
    This class is used to implement distributed locking in redis using streams and pub/sub api (For syncronous execution).
    """

    def __init__(
        self,
        haredis_client: HaredisClientCmdExecutor,
        ):
        ...
        self.__haredis_client = haredis_client
        self.__redis_logger = haredis_client.redis_logger
    ...
    def _thread_safe_lock_extend(self, lock: Lock, lock_time_extender_add_time: int, lock_time_extender_replace_ttl: bool):
    ...
    def xdel_event(
        ...
        time.sleep(delete_event_wait_time)
                
        delete_resp = self.haredis_client.client_conn.xdel(consumer_stream_key, event_id)
    ...
    def run_lte_streams(
        ...
        while is_locked:
            consume = self.haredis_client.client_conn.xread(streams=streams, count=1, block=lock_time_extender_blocking_time)
            ...
            if len(consume) > 0:
                ...
                if data == "end":
                    ...
                    _ = self.haredis_client.client_conn.xdel(lock_extend_stream_key, last_id)
                    self.redis_logger.debug("Lock Extender event deleted: {last_id}"
                                            .format(last_id=last_id))
                    _ = self._thread_safe_lock_extend(lock, lock_time_extender_add_time, lock_time_extender_replace_ttl)
                    return None
    ...
    def xconsume_call(
        ...
        result = self.haredis_client.xconsume(
            streams=streams,
            lock_key=lock_key,
        ...
        if "from-event" in result.keys():
            ...
            last_id, event_data = messages[0]
            data = event_data["result"]
            event_info = self.haredis_client.client_conn.xinfo_stream(consumer_stream_key)
    ...
    def run_lte_pubsub(
        ...
        while is_locked:
            ...
            ps = self.haredis_client.client_conn.pubsub()
            ps.subscribe(lock_extend_pubsub_key)
    ...
    def partial_main_selector_pubsub(
        self,
        func: Callable,
        lock_key: str,
        lock_time_extender_suffix: str,
    ...
    def subscriber_call(
        ...
        self.redis_logger.debug("Lock key: {lock_key} acquire failed. Result will be tried to retrieve from consumer".format(lock_key=lock_key))
        result = self.haredis_client.subscribe_msg(
            pubsub_channel=pubsub_channel,
            lock_key=lock_key,
            do_retry=consumer_do_retry,
            retry_count=consumer_retry_count,
